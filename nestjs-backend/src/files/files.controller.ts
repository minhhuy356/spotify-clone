...
export class FilesController {
  ...
  async uploadAndCreatePreview(
    ...
    const isRealVideo = metadata.streams.some(
      (s) => s.codec_type === 'video' && s.codec_name !== 'mjpeg',
    );

    const previewName = `preview-${Date.now()}${isRealVideo ? '.mp4' : '.m4a'}`;
    ...
    return new Promise((resolve, reject) => {
      const command = ffmpeg(inputPath)
        ...
        .on('end', () => {
          resolve({
            ...
            preview: previewName,
            url: `/tracks/preview/${previewName}`,
          });
        })
        .on('error', (err) => {
      ...
      if (isRealVideo) {
        ...
        command.audioCodec('aac').noVideo();
      }
