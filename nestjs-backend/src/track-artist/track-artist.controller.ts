...
import { TrackArtistQueryDto } from './dto/query.track-artist.dto';
...
export class TrackArtistsController {
  ...
  @Public()
  @Get('albumByArtist/:id')
  async fetchTracksByArtist(
    @Param('id') artistId: string,
    @Query() query: TrackArtistQueryDto,
  ) {
    return this.TrackArtistService.fetchAlbumsByArtist(artistId, query);
  }
}
