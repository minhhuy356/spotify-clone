...
export class UserActivitysController {
  ...
  @Post('album')
  @ResponseMessage('Subscribe Album')
  UserActivityArtist(
    @Body() createUserActivityDto: CreateUserActivitysDto,
    @User() user: IUser,
  ) {
  ...
  @Get('propose/:id')
  @Public()
  async fetchAlbumPropose(@Param('id') trackId: string) {
    return this.UserActivityService.fetchAlbumPropose(trackId);
  }

  @Get('artistAlsoLiked/:id')
  @Public()
  async fetchArtistAlsoLiked(@Param('id') artistId: string) {
    return this.UserActivityService.fetchArtistAlsoLiked(artistId);
  }
}
