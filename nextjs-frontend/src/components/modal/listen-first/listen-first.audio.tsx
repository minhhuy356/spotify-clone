...
const AlbumAudioListenFirst = ({ volume, setVolume, audioRef }: IProps) => {
  ...
  const src = currentTrack.videoListenFirstUrl
    ? `${backendUrl}${disk_tracks.videoListenFirst}${currentTrack.videoListenFirstUrl}`
    : `${backendUrl}${disk_tracks.audios}${currentTrack.audioUrl}`;

  return <audio ref={audioRef} loop src={src} preload="metadata" />;
};
