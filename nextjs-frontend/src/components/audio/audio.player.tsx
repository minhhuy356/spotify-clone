...
import { findIndexById } from "@/helper/context-menu/caculatePosition";
...
const AudioPlayer: React.FC<IAudioPlayerProps> = ({
  ...
  useEffect(() => {
    // Reset flag mỗi khi bài mới được load
    hasCalledApiRef.current = false;
    setTimeListened(0); // reset thời gian nghe
  }, [currentTrack]);
    ...
    if (audio) {
      const handleTimeUpdate = () => {
        setTimeListened((prevTime) => {
          ...
          const durationMin = ((currentTrack?.duration || 1) * 1) / 10;
          if (newTime === 1 && !hasCalledApiRef.current) {
            increaseCountView();
  ...
  }, [audioPlayerRef, currentTrack]);

  interface IncreaseCountViewParams {
    userId: string;
    body?: {
      trackId?: string;
      albumId?: string;
  ...
  const increaseCountView = async () => {
    if (currentTrack && session && playingSource.in) {
      let body: { trackId?: string; albumId?: string; artistId?: string } = {};

      if (playingSource.in === "artist") {
        body.artistId = playingSource._id;
      } else if (playingSource.in === "album") {
        body.albumId = playingSource._id;
      } else if (playingSource.in === "track") {
        body.trackId = playingSource._id;
      }
