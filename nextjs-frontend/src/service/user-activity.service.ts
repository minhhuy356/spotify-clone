...
export const fetchAlbumPropose = async (trackId: string) => {
  try {
    const res = await sendRequest<IBackendRes<IAlbum[]>>({
      url: `${backendUrl}${api_user_activity.propose}${trackId}`,
      method: "GET",
    });

    return res.data;
  } catch (error) {
    throw new Error("Lỗi khi lấy danh sách album đề xuất!");
  }
};

export const fetchArtistAlsoLiked = async (artistId: string) => {
  try {
    const res = await sendRequest<IBackendRes<IArtist[]>>({
      url: `${backendUrl}${api_user_activity.artistAlsoLiked}${artistId}`,
      method: "GET",
    });

    return res.data;
  } catch (error) {
    throw new Error("Lỗi khi lấy danh sách nghệ sĩ fan cũng thích!");
  }
};
