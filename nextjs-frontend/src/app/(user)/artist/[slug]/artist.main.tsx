...
import AvatarArtist from "./artist.avatar";
import CoverArtist from "./artist.cover";
...
const MainArtist = ({
  ...
  const handlePlayPause = (isPlayIn: boolean) => {
    if (isPlayIn) {
      dispatch(
        play({
      ...
      <div className="relative ">
        <div
          ...
          style={{
            ...
            opacity: Color(scrollCenter.color).isLight() ? 0.2 : 0.7,
          }}
        ></div>
        ...
        <div className="absolute top-0 z-20 w-full ">
          <div className="max-w-[1955px] mx-auto w-full">
                ...
                <div className="mr-8 cursor-pointer">
                  {isPlayInArtist ? (
                    <div onClick={() => handlePlayPause(false)}>
                      <ButtonPause size={isDesktop ? 1.15 : 1.1} />
                    </div>
                  ) : (
                    <div onClick={() => handlePlayPause(true)}>
                      <ButtonPlay size={isDesktop ? 1.15 : 1.1} />
                    </div>
                  )}{" "}
                </div>{" "}
            ...
            <div className="py-3 px-6 flex flex-col 4xl:flex-row gap-6">
              <div className={`w-[100%] ${chooseByArtist && "4xl:w-[60%]"}`}>
                <ArtistTableTrack
                  artist={artist}
