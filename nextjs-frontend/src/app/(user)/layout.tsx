...
import DiscographyHeaderPlay from "./artist/[slug]/discography/[type]/classify.discography.header.play";
import DiscographyHeaderArtist from "./artist/[slug]/discography/[type]/classify.discography.header.artist";
import Tooltip from "@/components/tooltip/tooltip";
import ClassifyDiscographyHeaderArtist from "./artist/[slug]/discography/[type]/classify.discography.header.artist";
import ClassifyDiscographyHeaderPlay from "./artist/[slug]/discography/[type]/classify.discography.header.play";
...
export default function RootLayout({
    ...
    const updateWidth = () => {
      ...
      if (screenWidth < 900) {
        ...
        if (layout.right.isClose) {
          ...
          const futureCenter =
            877.714 - hardRightWidth - minLeftWidth - space - 8;
  ...
  useEffect(() => {
    dispatch(setDiscography({ artist: null, albums: null }));
  }, [pathname]);
    ...
    const onMouseMove = (event: MouseEvent) => {
      ...
      if (screenWidth > 900) {
        if (side === "left") {
          ...
          if (layout.right.width <= minRightWidth + 10) {
            layout.right.isClose = true;
          } else {
            layout.right.isClose = false;
          }
        }
          ...
          >
            ...
            >
              <div className="w-full text-white h-full relative">
                {" "}
                <div
                  className={`absolute top-0 left-0 right-0 bg-base z-50 ${
                    scrollCenter.scroll !== 0 && "shadow-md shadow-black"
                  } header-center`}
                >
                  {discography.artist && discography.albums && (
                    <ClassifyDiscographyHeaderPlay
                      artist={discography.artist}
                    />
                  )}{" "}
                  <ClassifyDiscographyHeaderArtist />{" "}
                </div>
                <ScrollBar fatherRef={centerRef} position={"center"}>
                  {children}
                </ScrollBar>
              </div>{" "}
              <HeaderScroll />
