...
import AlbumListAblum from "../../../../components/list/list.data";
...
const AlbumMain = ({ album }: IProps) => {
  ...
  const [listDataRelated, setListDataRelated] = useState<IAlbum[]>([]);
  ...
  const fetchListAlbumRelated = async () => {
    ...
    // 1. Lấy các album liên quan
    const albums = await album_service.fetchAlbumByReleasedBy(
      album.releasedBy._id,
      { sort: "-countLike", limit: 15 }
    );

    if (!albums) return;

    // 3. Gọi setState hoặc xử lý tiếp albumsWithTracks
    setListDataRelated(albums);
  };
  ...
  const fetchTrackByAlbum = async () => {
    if (!album) return; // tránh lỗi khi chưa có slug
    try {
    ...
    <div className="w-full h-full relative">
      ...
      <div className="relative flex flex-col">
        ...
        <div className="absolute top-0 z-20 max-w-[1955px] mx-auto w-full px-1 right-0 left-0 flex flex-col gap-10 bg-inherit">
          ...
          {listDataRelated.length > 0 && (
            <div className="my-8">
              <ListData
                listDataRelated={listDataRelated}
                tData="album"
                title={`Album khác của ${album.releasedBy.stageName}`}
                textMore={<p>Xem danh sách đĩa nhạc</p>}
              />
            </div>
          )}
          <WebsiteInformation />
